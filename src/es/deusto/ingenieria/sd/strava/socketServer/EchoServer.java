...
public class EchoServer {
	...
	public static void main(String args[]) {
		if (args.length < 1) {
			System.err.println(" # Usando: TCPSocketEchoServer [PORT]");
			System.exit(1);
		}
		...
		try (ServerSocket tcpEchoServerSocket = new ServerSocket(serverPort);) {
			System.out.println(" - Facebook Server: Waiting for connections '" + tcpEchoServerSocket.getInetAddress().getHostAddress() + ":" + tcpEchoServerSocket.getLocalPort() + "' ...");
			...
			while (true) {
				...
				System.out.println(" - Facebook Server: New client connection accepted. Client Number: " + numClients++);
			}
		} catch (Exception e) {
			System.out.println("# Facebook Server: IO error:" + e.getMessage());
		}
	}
}